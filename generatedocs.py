...
# static int SteamUserStats_RequestCurrentStats(lua_State* L) {
...
def parse_field(line):
    ...
    parts = line.split(" ")
    field_name = parts[0]
    field_type = parts[1] if len(parts) >= 2 else "number"
...
def pop_line(lines, filetype, startswith = None, notstartswith = None, strip = True):
    line_token = entry_line_token(filetype)
    end_token = entry_end_token(filetype)
    original_line = lines.pop(0)
    ...
    if not line.startswith(line_token):
        ...
        return None
    line = line.removeprefix(line_token)
    if strip:
        line = line.strip()
    # ignore if line doesn't start with specified string
...
def process_entry(line, lines, filetype):
    ...
    while len(lines) > 0:
        ...
        # typed parameter
            ...
            while len(lines) > 0:
                next_line = pop_line(lines, filetype, startswith = "@field")
                if not next_line:
                    break
